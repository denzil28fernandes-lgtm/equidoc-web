    ...
    configureServer(server) {
      server.middlewares.use('/api/analyze', async (req, res) => {
        ...
        try {
          ...
          const { image, images, files, language } = JSON.parse(raw || '{}')
          // New clients send typed `files` ({ data, mime }); older ones send bare
          // base64 JPEG strings. Support both; PDFs arrive as application/pdf.
          ...
          if (fileParts.length === 0) return send(400, { error: 'no_image', message: 'No document was received.' })
